...
func (r InstallmentRepository) getQuerier(ctx context.Context) Querier {
...
// SaveAll persists multiple installments.
// When called within a TransactionManager.Run, the inserts run inside the parent transaction.
// Otherwise each insert is individually auto-committed by pgx.
...
func (r InstallmentRepository) DeleteByExpense(ctx context.Context, expenseID string) error {
	_, err := r.getQuerier(ctx).Exec(ctx, `DELETE FROM installments WHERE expense_id = $1`, expenseID)
	if err != nil {
