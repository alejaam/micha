...
export function ExpenseModal({
    ...
    const adminMember = useMemo(
        () => members.find((m) => m.user_id && String(m.user_id).trim() !== '') ?? null,
        [members]
    )
    const isCurrentUserAdmin = !!adminMember && defaultPaidByMemberId === adminMember.id

    const eligibleMembers = useMemo(
        () => {
            if (isCurrentUserAdmin) return members
            return members.filter((m) => m.id === defaultPaidByMemberId)
        },
        [members, isCurrentUserAdmin, defaultPaidByMemberId]
    )
    ...
    const selectedPaidByMember = eligibleMembers.find((member) => member.id === paidByMemberId) ?? null
    const isRegisteringForAnotherMember = !!defaultPaidByMemberId && paidByMemberId !== defaultPaidByMemberId
        ...
        <Modal title="New expense" onClose={onClose}>
            <form className="formStack" onSubmit={handleSubmit} noValidate>
                ...
                <FormField label="Paid by" htmlFor="modalPaidBy">
                    ...
                    {isCurrentUserAdmin && isRegisteringForAnotherMember && (
                        <p className="formHint formHintWarning">
                            As admin, you can register expenses for {selectedPaidByMember?.name || 'this member'} while their account is pending.
                            Once they activate their account, they should manage their own expenses.
                        </p>
                    )}
                ...
                <div className="modalActions">
                    ...
                    >
                        {isSubmitting
                            ? <><span className="spinIcon" aria-hidden /> Saving…</>
                            : 'Add expense'}
                    </button>
